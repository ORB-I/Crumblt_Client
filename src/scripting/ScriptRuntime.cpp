...
static int lua_workspace_getPart(lua_State *L) {
    ...
    auto **partPtr =
        (std::shared_ptr<SceneNode> **)lua_newuserdata(L, sizeof(std::shared_ptr<SceneNode> *));
...
static int lua_color3_fromRGB(lua_State *L) {
    ...
    lua_newtable(L);
    lua_pushnumber(L, r / 255.0f);
    lua_setfield(L, -2, "r");
    lua_pushnumber(L, g / 255.0f);
    lua_setfield(L, -2, "g");
    lua_pushnumber(L, b / 255.0f);
    lua_setfield(L, -2, "b");
    return 1;
}
    ...
    lua_pushcfunction(
        ...
        [](lua_State *L) -> int {
            ...
            if (strcmp(key, "Color") == 0) {
                lua_newtable(L);
                lua_pushnumber(L, node->colorR);
                lua_setfield(L, -2, "r");
                lua_pushnumber(L, node->colorG);
                lua_setfield(L, -2, "g");
                lua_pushnumber(L, node->colorB);
                lua_setfield(L, -2, "b");
                return 1;
            } else if (strcmp(key, "Position") == 0) {
                lua_newtable(L);
                lua_pushnumber(L, node->posX);
                lua_setfield(L, -2, "x");
                lua_pushnumber(L, node->posY);
                lua_setfield(L, -2, "y");
                lua_pushnumber(L, node->posZ);
                lua_setfield(L, -2, "z");
                return 1;
            } else if (strcmp(key, "Size") == 0) {
                lua_newtable(L);
                lua_pushnumber(L, node->sizeX);
                lua_setfield(L, -2, "x");
                lua_pushnumber(L, node->sizeY);
                lua_setfield(L, -2, "y");
                lua_pushnumber(L, node->sizeZ);
                lua_setfield(L, -2, "z");
                return 1;
            }
    ...
    lua_pushcfunction(
        ...
        [](lua_State *L) -> int {
            ...
            if (strcmp(key, "Color") == 0 && lua_istable(L, 3)) {
                lua_getfield(L, 3, "r");
                node->colorR = (float)lua_tonumber(L, -1);
                lua_pop(L, 1);
                lua_getfield(L, 3, "g");
                node->colorG = (float)lua_tonumber(L, -1);
                lua_pop(L, 1);
                lua_getfield(L, 3, "b");
                node->colorB = (float)lua_tonumber(L, -1);
                lua_pop(L, 1);
                node->dirty = true;
            } else if (strcmp(key, "Position") == 0 && lua_istable(L, 3)) {
                lua_getfield(L, 3, "x");
                node->posX = (float)lua_tonumber(L, -1);
                lua_pop(L, 1);
                lua_getfield(L, 3, "y");
                node->posY = (float)lua_tonumber(L, -1);
                lua_pop(L, 1);
                lua_getfield(L, 3, "z");
                node->posZ = (float)lua_tonumber(L, -1);
                lua_pop(L, 1);
                node->dirty = true;
            } else if (strcmp(key, "Size") == 0 && lua_istable(L, 3)) {
                lua_getfield(L, 3, "x");
                node->sizeX = (float)lua_tonumber(L, -1);
                lua_pop(L, 1);
                lua_getfield(L, 3, "y");
                node->sizeY = (float)lua_tonumber(L, -1);
                lua_pop(L, 1);
                lua_getfield(L, 3, "z");
                node->sizeZ = (float)lua_tonumber(L, -1);
                lua_pop(L, 1);
                node->dirty = true;
            }
...
void ScriptRuntime::loadScripts(const std::string &gameId) {
    ...
    if (!res.ok) {
        printf("[ScriptRuntime] Failed to load scripts\n");
        return;
    }

    auto j = json::parse(res.body);
    if (!j.value("success", false)) {
        printf("[ScriptRuntime] API error\n");
        return;
    }
