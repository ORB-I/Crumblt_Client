    ...
    if (res == CURLE_OK) {
        ...
        if (curl_easy_header(curl, "Set-Cookie", 0, CURLH_HEADER, -1, &h) == CURLHE_OK) {
            ...
            if (pos != std::string::npos) {
                ...
                std::string sid = val.substr(pos + 10, end == std::string::npos ? end : end - pos - 10);
...
HttpResponse Http::postForm(const std::string& url, const std::map<std::string, std::string>& fields) {
    std::string body;
    CURL* curl = makeCurl(url, body);
...
void Http::globalInit()    { curl_global_init(CURL_GLOBAL_DEFAULT); }
void Http::globalCleanup() { curl_global_cleanup(); }
