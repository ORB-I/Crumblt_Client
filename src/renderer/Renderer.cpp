...
void Renderer::render(const Scene& scene, const std::vector<Player>& players, int winW, int winH) {
    ...
    int startX = cx % gridSize; if (startX > 0) startX -= gridSize;
    int startY = cy % gridSize; if (startY > 0) startY -= gridSize;
    for (int x = startX; x < winW; x += gridSize) SDL_RenderDrawLine(m_r, x, 0, x, winH);
    for (int y = startY; y < winH; y += gridSize) SDL_RenderDrawLine(m_r, 0, y, winW, y);
...
void Renderer::drawPlayer(const Player& p, int cx, int cy) {
    ...
    // Fill
    SDL_SetRenderDrawColor(m_r,
        (uint8_t)(p.colorR * 255),
        (uint8_t)(p.colorG * 255),
        (uint8_t)(p.colorB * 255), 255);
